...
int Preemptive_Priority(int num, int s_num, int k, float lamda_ar, float lamda_b, float lamda_l)
{
    ...
    for (int time = 0; time < 3000; time += 1)
    {
        ...
        for (int i = 0; i < num; i++)
        { // when time==arrival time push to queue
        ...
        { // when all processes are terminated break
            break;
        }
        ...
        for (int i = 0; i < s_num; i++)
        { // check for the process with maximum priority locking a semaphore
            if (s[i]->value == 0)
            {
        ...
        if (!Q.empty())
        {
            ...
            if (Q.top()->get_priority() < temp_p && (Q.top()->priority) > 0)
            { // check if priority in ready queue is greater than priority of proc in sems

                temp = Q.top();
                temp->running = 1;

                if (propability(k) && temp->lifetime > 1)
                { // process tries to execute down()
            ...
            { // if process finises up() then push it back to the ready queue
                s_temp->state = "READY";
                ...
                if (s[sem_index]->isEmpty())
                { // if sem queue is empty
                    s[sem_index]->value = 1;
        ...
        else if (Q.empty() && temp_p != 10)
        { // if queue is empty but sems have processes
        ...
        { // if queue is empty and no proc in temps
            continue;
        }
        ...
        for (int i = 0; i < num; i++)
        { // select waiting and blocking time from each process
            ...
            { // check if priority in ready queue is greater than priority of proc in sems
                ...
                { // process tries to execute down()
                    ...
                    else
                    {
                        ...
                        temp->blocked = 1;
                        s[random_s]->push(temp);
                        if (temp->priority < s[random_s]->curr_process->priority)
